...
struct ReprojectionError {
    ...
    bool operator()(const T *const camera, const T *const point, T *residuals) const {
        /*
         * "camera" contains the R and t (rigid-body transformations) parameters of a camera w.r.t. the base camera
         * "point" is a 3D point in the scene in world reference frame
         */

        //3D point "p" is "point" w.r.t. the "camera" frame. It is used to compute the 2D predicted point projection
        T p[3];

        //rotation vector
        T r_vec[3];

        //extract rotation vector from camera
        r_vec[0] = T(camera[0]);
        r_vec[1] = T(camera[1]);
        r_vec[2] = T(camera[2]);
        ...
        //prediction of 2D point projection from "p" (we are dealing with a canonical camera)
        T predicted_x = p[0] / p[2];
        T predicted_y = p[1] / p[2];

        //projection error among prediction and observation
...
//IO AVEVO QUESTO NEL CODICE SCARICATO
...
void BasicSfM::solve() {
    ...
    while (!seed_found) {
        ...
        cv::findHomography(points0, points1, inlier_mask_H, cv::RANSAC, 1.0);

        // calculate the essential matrix
        cv::Mat essential = cv::findEssentialMat(points0, points1, intrinsics_matrix, cv::RANSAC, 0.999, 1.0,
                                                 inlier_mask_E);

        // check the number of inliers
        int tot_inlier_H = cv::sum(inlier_mask_H)[0];
        int tot_inlier_E = cv::sum(inlier_mask_E)[0];

        //DEGUG: std::cout << "count e " << tot_inlier_E << " count h " << tot_inlier_H << std::endl;

        // check if model E has more inliers than model H
        if (tot_inlier_E > tot_inlier_H) {
            //DEBUG: std::cout << "[INFO] inliers essential " << tot_inlier_E << " inliers homography " << tot_inlier_H << std::endl;
            // decompose essential matrix in order to have Rotation and translation
            cv::recoverPose(essential, points0, points1, intrinsics_matrix, init_r_mat, init_t, inlier_mask_E);

            // set to true
            seed_found = true;

            // remember only inliers
            std::vector<cv::Point2d> points0_rem;
            std::vector<cv::Point2d> points1_rem;
            std::vector<int> inliers = (std::vector<int>) inlier_mask_E;
            for (int i = 0; i < inliers.size(); i++) {
                if (inliers[i]) {
                    points0_rem.push_back(points0[i]);
                    points1_rem.push_back(points1[i]);
                }
            }
            points0.swap(points0_rem);
            points1.swap(points1_rem);
        }
    }
...
void BasicSfM::bundleAdjustmentIter(int new_cam_idx) {
    ...
    options.num_threads = 16; // metti poi 4
    ...
    while (keep_optimize) {
        ...
        for (int i_obs = 0; i_obs < num_observations_; i_obs++) {
            ...
            if (pose_optim_iter_[pose_index_[i_obs]] > 0 && pts_optim_iter_[point_index_[i_obs]] > 0) {
                ...
                //observed 2D points
                double observed_x = observations_[i_obs*2];
                double observed_y = observations_[i_obs*2+1];

                //camera parameters related to the 2D observed point
                double* camera_ptr = cameraBlockPtr(pose_index_[i_obs]);
                //3D point related to the 2D observed point
                ...
                problem.AddResidualBlock(cost_function, new ceres::CauchyLoss(2*max_reproj_err_), camera_ptr, point_ptr);
            }
        }
...
//      std::cout << summary.FullReport() << "\n";
        ...
        if (cheirality_violation) {
            std::cout << "****************** OPTIM CHEIRALITY VIOLATION for " << dbg_n_violations
