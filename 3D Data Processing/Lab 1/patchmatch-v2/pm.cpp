...
#pragma omp parallel for
  ...
  void PatchMatch::spatial_propagation(int x, int y, int cpv, int iter) {
    ...
    //sliding direction:
    //  even iteration -> check left and top pixel
    //  odd iteration --> check right and bottom pixel
    int dir = (iter%2 == 0) ? -1 : 1;

    //check if left or right pixel (depending on iteration) provides a disparity that decrease the matching cost

    //check if the left or right pixel is inside the image
    if(inside(x+dir, y, 0, 0, cols_, rows_)){
      //retrive the actual cost for pixel (x,y)
      float cost = costs_[cpv].at<float>(y,x);

      //retrive candidate new disparity from neighbour pixel
      float new_disp = disps_[cpv].at<float>(y,x+dir);

      //retrive candidate new cost for pixel (x,y) with the candidate new disparity
      float new_cost = precomputed_disp_match_cost(new_disp, x, y, cpv);

      //if new cost is smaller, update disparity and cost maps with new values
      if (new_cost < cost){
        disps_[cpv].at<float>(y,x) = new_disp;
        costs_[cpv].at<float>(y,x) = new_cost;
      }
    }

    //SAME AS BEFORE, BUT
    //check if TOP or BOTTOM pixel (depending on iteration) provides a disparity that decrease the matching cost

    //check if the top or bottom pixel is inside the image
    if(inside(x, y+dir, 0, 0, cols_, rows_)){
      //retrive the actual cost for pixel (x,y)
      float cost = costs_[cpv].at<float>(y,x);

      //retrive candidate new disparity from neighbour pixel
      float new_disp = disps_[cpv].at<float>(y+dir,x);

      //retrive candidate new cost for pixel (x,y) with the candidate new disparity
      float new_cost = precomputed_disp_match_cost(new_disp, x, y, cpv);

      //if new cost is smaller than the actual cost, update disparity and cost maps
      if (new_cost < cost){
        disps_[cpv].at<float>(y,x) = new_disp;
        costs_[cpv].at<float>(y,x) = new_cost;
      }
  ...
  void PatchMatch::view_propagation(int x, int y, int cpv) {
    ...
    //IF current view is the left one (cpv=0), we have to check if pixels in the right view (cpv=1) have a match with
    //the current one (x,y), therefore we need to deal with positive disparity(right to left case)
    //ELSE the opposite (left to right case)
    int disp_sign = (!cpv == 0) ? -1 : 1;

    //check all pixel along one row in the other view (y is fixed)
    for(int col = 0; col < cols_; col++){
      //retrive, form the other view, pixel disparity (that will be the new candidate disparity)
      float new_disp = disp_sign*disps_[!cpv].at<float>(y, col);

      //check if pixel (col, y) from the other view has a disparity that match pixel (x,y) in the current view
      if(x == static_cast<int>(col + new_disp)){
        //reset the disparity to a positive value
        new_disp *= disp_sign;

        //current cost of pixel (x,y) in the current view
        float cost = costs_[cpv].at<float>(y,x);

        //candidate cost of pixel (x,y) in current view with new candidate disparity
        float new_cost = precomputed_disp_match_cost(new_disp, x, y, cpv);

        //if the new cost is smaller than the actual cost, update the cost and the disparity maps
        if(new_cost < cost){
          disps_[cpv].at<float>(y,x) = new_disp;
          costs_[cpv].at<float>(y,x) = new_cost;
        }
  ...
  void PatchMatch::disp_perturbation(int x, int y, int cpv, float max_delta_z, float end_dz) {
    ...
    //IF left view (cpv=0), we need to deal with negative disparities (left to right case)
    //ELSE the opposite (right to left case)
    int disp_sign = (cpv == 0) ? -1 : 1;

    //continue the perturbation until the interval among max_delta_z ad end_dz is less than 0
    while(max_delta_z > end_dz){

      float upper = max_delta_z;
      float lower = -max_delta_z;

      //retrive current disparity
      float disp = disps_[cpv].at<float>(y, x);

      //check if the disparity perturbation is always a valid perturbation, if not fix it
      if(disp + upper > MAX_DISPARITY){
        upper = MAX_DISPARITY - disp;
      }
      if(disp + lower < 0){
        lower = - disp;
      }

      //generate a random delta_z in the interval passed as parameter
      float delta_z = random_generator.uniform(lower, upper);

      //compute the new candidate disparity
      float new_disp = disp + delta_z;

      //compute the matching pixel in the other view with the perturbed disparity
      int match_x = static_cast<int>(x + disp_sign*new_disp);

      //check if the perturbation maps a pixel that resides inside the image, otherwise we fix the boudaries
      if(!inside(match_x, y, 0, 0, cols_, rows_)){
        //CASE 1: pixel outside on the left side of the image and left view
        if(match_x < 0 && cpv == 0){
          upper = x - disp;
        }
        //CASE 2: pixel outside on the left side of the image and right view
        else if(match_x < 0 && cpv == 1){
          lower = -(x + disp);
        }
        //CASE 3: pixel outside on the right side of the image and left view
        else if(match_x >= cols_ && cpv == 0){
          lower = -(cols_ + disp - x);
        }
        //CASE 4: pixel outside on the right side of the image and right view
        else{
          upper = cols_ - x - disp;
        }

        //recompute the perturbation and it's matching pixel as before
        delta_z = random_generator.uniform(lower, upper);
        new_disp = disp + delta_z;
        match_x = static_cast<int>(x + disp_sign*new_disp);

        //if matching pixel is still not inside the image, fix the disparity
        if(!inside(match_x, y, 0, 0, cols_, rows_)){
          //check if the pixel is outside on the right or left side of the image and fix disparity accordingly
          if(x + disp_sign*disp < 0){
            disp = random_generator.uniform(0, x);
          }
          else{
            disp = random_generator.uniform(0, cols_ - x);
          }
          
          //update disparity e cost maps
          float cost = precomputed_disp_match_cost(disp, x, y, cpv);
          disps_[cpv].at<float>(y,x) = disp;
          costs_[cpv].at<float>(y,x) = cost;
          continue;         
        }
      }
      ...
      //retrive the actual cost for pixel (x,y)
      float cost = costs_[cpv].at<float>(y,x);
      //compute matching cost for pixel (x,y), with the new disparity; this is the candidate new cost
      float new_cost = precomputed_disp_match_cost(new_disp, x, y, cpv);
